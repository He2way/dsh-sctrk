...
// v77 布局：轨迹条移到左侧——轨道 left:8，刻度/簇点吸附左缘、伸长向右，
//   浮窗/桥接带镜像到轨道右侧，交互带改在轨道右侧。
// v78 定位：轨道左侧跟随**对话窗口**左缘（rect.left + 8，非整页左缘）——
//   measure 记录容器 left；resize 监听 + 500ms 自愈 tick 廉价跟随布局变化。
// ============================================================================
...
.dsh-sctMark{
  position:absolute;left:1px;width:7px;height:1px;border-radius:0;z-index:1;
...
.dsh-sctCluster{
  position:absolute;left:1px;min-width:10px;border-radius:0;z-index:1;
  transform-origin:left center;will-change:transform,opacity;
  transition:transform .45s cubic-bezier(.22,1,.36,1),opacity .35s ease,filter .2s ease;
}
...
.dsh-sctFloat{
  ...
  transition:top .18s cubic-bezier(.22,1,.36,1),left .18s cubic-bezier(.22,1,.36,1),opacity .15s ease;
...
`;
    ...
    function ScrollTrack(props) {
      ...
      const trySnap = (x, y) => {
        ...
        if (x < r.left - 4 || x > r.right + SNAP_LEFT || y < r.top || y > r.bottom) return false
      ...
      const handlePosition = (x, y) => {
        ...
        if (!trackEl) return
        const r = trackEl.getBoundingClientRect()
        const inBand = x >= r.left - 4 && x <= r.right + SNAP_LEFT && y >= r.top && y <= r.bottom
        if (inBand) {
          trySnap(x, y)
          cancelHide()
          return
      ...
      React.useEffect(() => {
        ...
        const measure = () => {
          ...
          setModel({
            rect: { top: rect.top, height: rect.height, left: rect.left },
        ...
        attach()
        // v78：轨道左侧跟随对话窗口左缘（布局变化/窗口缩放时廉价更新，不整表重测）
        const followLeft = () => {
          if (disposed) return
          const el = document.querySelector('[data-conversation-scroll]')
          if (!el) return
          const nl = el.getBoundingClientRect().left
          setModel((m) => {
            if (m === null || Math.abs(nl - m.rect.left) < 0.5) return m
            return { ...m, rect: { ...m.rect, left: nl } }
          })
        }
        window.addEventListener('resize', followLeft)
        ...
        return () => {
          ...
          if (ro) ro.disconnect()
          if (mo) mo.disconnect()
          window.removeEventListener('resize', followLeft)
      ...
      const onPointerDown = (e) => {
        ...
        const r = trackEl.getBoundingClientRect()
        if (e.clientX > r.left + 8 || e.clientX < r.left) return
      ...
      if (hoverInfo !== null) {
        ...
        const left = 1 + stretched + FLOAT_GAP
        const maxLeft = Math.max(0, (window.innerWidth - FLOAT_W) - 8)
        const floatLeftPx = Math.min(left, maxLeft)
        let titleText = ''
        if (hoverInfo.kind === 'mark') {
        ...
        children.push(React.createElement('div', {
          ...
          style: {
            top: (floatTopPx - 4) + 'px',
            left: '8px',
            width: Math.max(0, floatLeftPx - 8) + 'px',
            height: (estH + 8) + 'px'
          }
        }))
        const floatChildren = []
        ...
        children.push(React.createElement('div', {
          ...
          style: { top: floatTopPx + 'px', left: floatLeftPx + 'px' }
        }, floatChildren))
      }
